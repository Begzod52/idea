...
const Header = () => {
  ...
  // Блокируем/возвращаем прокрутку сайта при открытом каталоге
  useEffect(() => {
      ...
      >
              ...
              {isLangOpen && (
                <div className="absolute top-full right-0 mt-3 w-72 bg-white rounded-2xl shadow-lg p-6 z-20 border border-gray-100">
                  ...
                  <h3 className="text-xl font-bold mb-6 text-gray-900">
                    {t("header.select_language", "Выберите язык")}
                  </h3>
                  <ul className="space-y-5">
                    {Object.entries(languages).map(([name, flagUrl]) => (
                      <li
                        key={name}
                        onClick={() => onChangeLang(name)}
                      ...
                      >
                        <div
                          ...
                          style={{
                            borderColor:
                              selectedLang === name ? "#e83e8c" : "#d1d5db",
                          }}
                        >
                          {selectedLang === name && (
                            <div className="w-2.5 h-2.5 bg-[#e83e8c] rounded-full" />
                          )}
                        </div>
                        <img
                          src={flagUrl}
                        ...
                        />
                        <span className="text-gray-800 group-hover:text-pink-600 font-medium text-base">
                          {name}
                        </span>
                      </li>
                    ))}
        ...
        <div className="flex items-center justify-between gap-8 py-5 px-[5%] bg-white">
          ...
          <button
            ...
            className="relative flex items-center gap-2.5 bg-pink-600 cursor-pointer rounded-lg text-white font-bold text-base py-3 px-5 hover:bg-pink-700 transition-colors"
            ...
            <span className="relative inline-block w-5 h-5">
              <Menu
                ...
                className={`absolute inset-0 transition duration-200 ${
                  catalogOpen ? "opacity-0 scale-50 rotate-90" : "opacity-100"
                }`}
              />
              <X
                size={20}
                className={`absolute inset-0 transition duration-200 ${
                  catalogOpen ? "opacity-100" : "opacity-0 scale-50 -rotate-90"
                }`}
              />
            </span>
            <span>{t("catalog.button", "Каталог товаров")}</span>
          </button>
          ...
          <div className="flex items-center flex-grow bg-gray-100 rounded-lg px-4 focus-within:ring-2 focus-within:ring-pink-500">
            <input
              ...
              className="w-full bg-transparent border-none p-3 text-base focus:outline-none"
            />
          ...
          <div className="flex items-center gap-6" ref={rightRef}>
            <Link
              to="/favorites"
              className="relative flex flex-col items-center text-xs gap-1 text-gray-600 hover:text-pink-600 font-medium"
            >
              <Heart size={24} />
              {favorites.length > 0 && (
                <span className="absolute top-[-5px] right-[5px] flex items-center justify-center w-[18px] h-[18px] bg-[#fecb3e] text-black text-[11px] rounded-full font-medium border-2 border-white">
                  {favorites.length}
                </span>
              )}
              {t("header.favorites")}
            ...
            >
              <ShoppingBag size={24} />
              {cart.length > 0 && (
                <span className="absolute top-[-5px] right-[5px] flex items-center justify-center w-[18px] h-[18px] bg-[#fecb3e] text-black font-bold text-[11px] rounded-full border-2 border-white">
                  {cart.length}
                </span>
              )}
              {t("header.cart")}
