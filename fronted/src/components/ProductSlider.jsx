      ...
      <div className="px-[5%] py-4">
        <div className="flex justify-between items-center mb-6">
          ...
          <button className="text-pink-600 font-medium flex items-center gap-2 cursor-pointer hover:text-pink-800 transition-colors">
            {t("slider.view_all")} <ChevronRight size={20} />
          </button>
        </div>
          ...
          >
            {products.map((product) => (
              <SwiperSlide key={product.id} className="h-auto pb-2">
                <ProductCard product={product} sliderName="slider" />
              </SwiperSlide>
            ))}
          </Swiper>
          <div className="product-swiper-button-prev absolute top-1/2 -left-10 -translate-y-1/2 z-10 cursor-pointer bg-white rounded-full shadow-lg p-2 hover:border-pink-600 border transition-colors">
            <ChevronLeft className="text-pink-600" size={28} />
          </div>
          <div className="product-swiper-button-next absolute top-1/2 -right-10 -translate-y-1/2 z-10 cursor-pointer bg-white rounded-full shadow-lg p-2 hover:border-pink-600 border transition-colors">
